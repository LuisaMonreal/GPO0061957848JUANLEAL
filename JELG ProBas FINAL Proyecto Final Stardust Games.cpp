...
int main()
{
	...
	case 6://Salir del programa
		printf("\n Muchas gracias por utilizar este programa :D, hasta la proxima!!!\n");
		system("pause");
		archivo();
		exit(1);
		break;
...
{
	int eliminar2;
	printf("ingrese el numero de articulo del articulo a eliminar\n");
	scanf_s("%d", &eliminar2);
	...
	for (int i = 0; i < registros; i++)
	{
		...
		{
			numart[i] = 0;
			printf("n\nEl articulo ya ha sido eliminado\n");
		}
	}
}
	...
	case 1://Buscar por clasificacion
		...
		for (int i = 0; i < registros; i++)
		{
			if (clasificacion[i] == busqueda1)
			{
				if (numart[i] == 0)
				{
					printf("\nEl articulo ya no existe, ha sido eliminado\n");
				}
				else
				{
